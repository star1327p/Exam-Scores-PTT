...
- **True positive**: The datapoint is actually positive and is predicted as positive, so the model correctly predicts the positive outcome.
- **True negative**: The datapoint is actually negative and is predicted as negative, so the model correctly predicts the negative outcome.
- **False positive**: The datapoint is actually negative but is predicted as positive, so the model **in**correctly predicts the positive outcome, i.e., false alarm.
- **False negative**: The datapoint is actually positive but is predicted as negative, so the model **in**correctly predicts the negative outcome, i.e., error.
...
Precision and recall are two widely-used metrics to measure the accuracy of the prediction model. Precision is defined as the percentage of true positives among all datapoints the model predicted to be positive. In our example, precision is the percentage of the respondents who actually got **College_Score** at least 65, among the respondents we predicted to make this achievement given their **HighSchool_PR**. 

$$\text{precision} = \dfrac{\text{True Positive}}{\text{Predicted Positive}} = \dfrac{\text{True Positive}}{\text{True Positive + False Positive}}$$
\textcolor{red}{Add the numbers and numeric formulas}    
...
Add the numbers and numeric formulas  

Also write about false positive rate and false negative rate.  

Relationship between fpr, fnr, precision, recall.
...
# Don't just write the syntax like confusion[1,1]. This is really confusing!
...
For the next steps in learning statistics, we recommend reading *The Statistical Sleuth: A Course in Methods of Data Analysis* \cite{ramsey2013statistical}, which is the textbook for undergraduate-level regression analysis at Duke Statistical Science.^[<https://www2.stat.duke.edu/courses/Fall18/sta210.001/>] The book covers topics such as ANOVA (Analysis of Variance) and multiple linear regression. It also provides data files for case studies and exercises.^[<http://www.statisticalsleuth.com/>]  
