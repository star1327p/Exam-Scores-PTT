...
Need to elaborate on the first paragraph. Cannot just write one line.  
...
\textcolor{red}{Add a foreword paragraph?!}    

This document is created with $\mathsf{R}$ Markdown \cite{r-markdown-pkg} in RStudio.^[<https://rmarkdown.rstudio.com/articles_intro.html>] $\mathsf{R}$ Markdown has a framework to directly incorporate code and graphs in the report. This not only reduces the errors from copy-pasting tables and plots, but also ensures reproducibility of the data analysis \cite{baumer2014r-markdown}. $\mathsf{R}$ Markdown can generate reports in PDF, HTML, or Microsoft Word formats. $\mathsf{R}$ Markdown also provides a template for presentation slides, and the output can be in HTML, PDF, or Microsoft PowerPoint files. For a full book with multiple chapters, we recommend using the $\mathsf{R}$ package `bookdown` \cite{r-bookdown} for a comprehensive and reproducible workflow.  
...
\textcolor{red}{Talk about Python, `numpy`, and `scikit-learn`, etc?!}  
