...
Precision and recall are two widely-used metrics to measure the performance of the prediction model. Precision is defined as the percentage of true positives among all datapoints the model predicted to be positive. In our example, precision is the percentage of the respondents who actually got **College_Score** at least 65, among the respondents we predicted to make this achievement given their **HighSchool_PR**.  
...
\textcolor{red}{Interpretation here}

The model is imperfect.  

Also write about the overall accuracy.  
...
Note that when the data are imbalanced (say, 80% of the records belong to one category), accuracy is not a good measure of model performance.^[https://www.analyticsvidhya.com/blog/2017/03/imbalanced-data-classification/]  

The good news is that by setting the classification threshold to ... $\Rightarrow$ the data are relatively balanced (insert percentage here).
