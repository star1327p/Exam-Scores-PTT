...
```{r confusion-subset}
# UNFINISHED HERE
# HS0to79_ind has only FALSE predicted outcomes.
# HS80to89_ind has only FALSE predicted outcomes.
# HS95to99_ind has only TRUE predicted outcomes.

# Only HS90to94 has both TRUE and FALSE predicted outcomes.
...
Test to make the confusion matrix 2x2

```{r test-confusion-code}
# Should build this into the confusion_subset function, as a toggle feature.
# dim(confusion_0to79) # 2x1
#  dimnames(confusion_0to79) # Check this first.

# colnames(confusion_0to79) # predicted: "FALSE" only

# UNFINISHED: To be organized
# yt = as.table(cbind(confusion_0to79,c(0,0)))
# colnames(yt) = c("FALSE","TRUE")
# names(dimnames(yt)) = c("act","pred")
# yt
```
...
```{r test-confusion-extreme}
# Should build this into the confusion_subset function, as a toggle feature.
# dim(confusion_0to79) # 2x1
#  dimnames(confusion_0to79) # Check this first.

# colnames(confusion_95to99) # predicted: "TRUE" only

# UNFINISHED: To be organized
# xyt = as.table(cbind(c(0,0), confusion_95to99))
# colnames(xyt) = c("FALSE","TRUE")
# names(dimnames(xyt)) = c("act","pred")
# xyt
```
