...
Since we concluded in Section \ref{linear-reg} that it is inappropriate to perform a linear regression between **HighSchool_PR** and **College_Score**, let's try a different statistical model to evaluate the relationship between the two variables.  

What is the problem statement? Can be changed.  
...
Unsure if the section name should contain the term "Advanced Content", because we try to explain the logistic regression in basic terminology.  

We moved the model validation sections to a separate chapter. The machine learning concepts are not related to the logistic regression itself, so readers without a mathematical background can still learn these concepts.  

In the model validation sections, we can regard the logistic regression model as a blackbox, which simply produces the binary classification output.  
...
\section{\textcolor{red}{Model Validation}}
...
Give the example of sensitivity vs specificity in clinical testing.  
...
Assume 0.1% of the population have a specific disease.  

In a population of 500,000 people, 500 people would have the disease.  

Now we have a medical test that claims to be 99% accurate, which means the test has 99% sensitivity and 99% specificity. Hence the false positive rate and the false negative rate are both 1%.   

For the 500 people who actually have the disease, 495 people tested positive and 5 people tested negative.  
...
Given that a patient tested positive, what is the probability that he/she actually have the disease?  
...
Therefore, the patient has only 9.16% of chance of actually having the disease, despite that he/she already tested positive.  

The patient does not have the disease for more than 90% of the time.  

\textcolor{red}{Interpretation of the numbers here}
