...
\textcolor{red}{Some context here}

To measure the model performance, we start with the overall **accuracy** to calculate how many datapoints the model predicted correctly. A datapoint is correctly predicted if one of the two scenarios occurs:  
...
\textcolor{red}{More context here}  

**Precision** and **recall** are also two widely-used metrics to measure the performance of the prediction model. Precision is defined as the percentage of true positives among all datapoints the model predicted to be positive. In our example, precision is the percentage of the respondents who actually got **College_Score** at least 65, among the respondents we predicted to make this achievement given their **HighSchool_PR**.  
