...
\subsubsection{Model Validation: In-Sample Prediction}
...
Let's create a contingency table to show the comparison between the actual outcomes and predicted outcomes, i.e., whether each respondent obtained **College_Score** at least 65 or not.
...
# Contingency table
contingency = table(actual_65up, predicted_65up)
# revert the order of FALSE and TRUE
contingency = contingency[2:1, 2:1] 
contingency
```

Below is the percentage version of the contingency table.
...
Create a table to show the true positives, true negatives, false positives, and false negatives.  
...
rownames(table_df) = c("-----------","Actual True","------------","Actual False")
colnames(table_df) = c("|","Predicted True","|","Predicted False")

table_df
```


\textcolor{red}{Formulas: precision, recall}

\subsubsection{Model Validation: Breakdown by High School Entrance Exam Scores}
