...
Let's implement the logistic regression model in `R` as below.
...
The `Pr(>|z|)` is the p-value of each independent variable, and we can see that **HighSchool_PR** is statistically significant. When **HighSchool_PR** increases by one, the log odds $\log (\dfrac{p}{1-p})$ of getting **College_Score** at least 65 increases by approximately 0.15. After transforming log odds $\log (\dfrac{p}{1-p})$ back to odds $\dfrac{p}{1-p}$, we get that the odds are multiplied by $e = \exp(0.15) \approx 1.161$. In other words, when **HighSchool_PR** increases by one, the odds of getting **College_Score** at least 65 increases by approximately 16.1%. 

\section*{\textcolor{red}{Unfinished below}}

Find all available outputs from the model.

```{r objects-model}
objects(model)
```

For better reproducibility of coefficients
...
Intercept: Interpretation also needed  
...
Linear regression is widely used, but it is not a panacea for data analysis. The model assumptions need to be met in the data, as stated in Section ???.   
