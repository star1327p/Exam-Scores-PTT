...
The most common form of hypothesis testing is to check whether a coefficient is zero or not in linear regression, but this is by no means the only form. For categorical data, the Pearson's chi-squared test ($\chi^2$) can be used to evaluate whether the categorical counts of **HighSchool_PR** and **College_Score** are due to random chance. The test statistic $\chi^2$ is defined as below, and it asymptotically approaches a $\chi^2$ distribution.  
...
It is tedious to manually calculate the Pearson's chi-squared test, so we use the function `chisq.test` in the `R` package `stats`. The results show p-value < 0.05, so we can reject $H_0$ and conclude that a positive association exists between the categorical counts of **HighSchool_PR** and **College_Score**. But we should still try to understand the rationale behind the `chisq.test` function to verify that we are using it correctly. For example, the test statistic $\chi^2$ has `df = 9` degrees of freedom, because `df` is computed as $(4-1)\times(4-1)$.   
...
\textcolor{red}{Look at what chisq.test provides us}  

The observed counts are the data in `table_4x4`, as shown earlier in Section \ref{bivariate-top-scorers}.  

```{r chi-squared-observed}
print(results$observed)
```

The expected counts are simulated and conditioned on row and column totals.  
...
\textcolor{red}{Also need to calculate and explain the test statistic by hand}  
...
With 16 cells, $E_i = 188/16 = 11.75$  $\Rightarrow$ \textcolor{red}{Wrong!}  

We need to adhere to the row and column totals.  
