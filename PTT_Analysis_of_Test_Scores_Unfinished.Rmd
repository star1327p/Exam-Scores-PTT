...
Now we summarize the results in k-fold cross-validation, i.e., combine all 10 confusion matrices into one.  
...
After obtaining the k-fold cross validation results in a single confusion matrix, we can calculate the accuracy, precision, recall, FPR, FNR using the `confusion_to_measures` function from the previous section.
...
Also consider: Different random seeds for the partition splits.  

Unsure whether we need to do the full implementation here, but we should at least write an explanation for this.
