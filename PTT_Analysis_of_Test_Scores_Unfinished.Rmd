...
# Additional Resources for Learning {#resources}
...
In the scatterplot from Section \ref{bivariate}, the red dot in the top-right indicates where the author's high school and college entrance exam scores stand. We also added the reference lines for **HighSchool_PR** 80 and **College_Score 60**. **HighSchool_PR** 95 and **College_Score** 69 are definitely above-average scores, despite imperfect.      
...
\textcolor{red}{Consider adding subsection headers to this chapter}   

Let's examine all of the **College_Score** datapoints given **HighSchool_PR** 95:    

```{r HighSchool-PR-95}
sort(data_corr$College_Score[which(data_corr$HighSchool_PR == 95)])
```

There are seven points in the data, with median 68 and mean about 67.6. One of them is **College_Score** 69, the same score as the author's. (She personally did not respond to this survey in 2015 because she discovered this announcement a few years later.)  

Let's also examine the nearby values, i.e., **HighSchool_PR** 94 and 96.  

Look at **HighSchool_PR** 94  

```{r HighSchool-PR-94}
sort(data_corr$College_Score[which(data_corr$HighSchool_PR == 94)])
```

Ten datapoints: The median is 66, and the mean is 63.  

Look at **HighSchool_PR** 96  

```{r HighSchool-PR-96}
sort(data_corr$College_Score[which(data_corr$HighSchool_PR == 96)])
```

Five datapoints: The median is 67, and the mean is 67.4.  

Look at **HighSchool_PR** 97-99  

(Although this is an unfair comparison, we are still interested in the outcomes.)  
...
It is somewhat unfair to compare **HighSchool_PR** 95 with **HighSchool_PR** 97 as a nearby value, because the most prestigious high schools in Taipei typically requires **HighSchool_PR** 97 or higher.^[<https://cclccl-life.blogspot.com/2013/06/blog-post_9.html>] Students with **HighSchool_PR** 97 may receive significantly more resources than the ones with **HighSchool_PR** 95, despite the difference is only two percentage points.  

Add: Observe the graph $\Rightarrow$ **College_Score** 69 is about the middle of the **College_Score** datapoints for **HighSchool_PR** 97-99.  

Should I incorporate the median and mean for this group here?  

Write something more, including the logistic regression prediction  
...
Given the author's **HighSchool_PR** 95 score, the predictive probability is about 64.6% in terms of how likely she would have achieved **College_Score** at least 65.  

As a reference, Section \ref{logit-point-est} shows that given **HighSchool_PR** 99, the estimated probability to achieve this is 76.9%.  

More interpretation here?  

Impostor syndrome $\Rightarrow$ compensated by studying extra hard  

The numbers made the author feel better, because she still had a good chance of getting **College_Score** at least 65 given her original **HighSchool_PR**.  
