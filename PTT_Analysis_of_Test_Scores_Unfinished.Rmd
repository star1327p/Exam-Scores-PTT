...
```{r latex-cite-command, includ=FALSE}
# %\let\cite\citep
...
However, the goal of building this model is to optimize the performance for future input, i.e., incoming students who just obtained their **HighSchool_PR** scores. In-sample prediction is insufficient because we need to test on unseen data to avoid overfitting.^[<https://elitedatascience.com/overfitting-in-machine-learning>] But why is overfitting bad? Because the model would do well on the existing data but perform poorly on the new data, which is undesirable. This is similar to a student who memorizes the answers to score 100% on quizzes without understanding the actual content. Then this student may not do well on the final exam because he/she has not seen the questions before. Thus, we need to perform out-of-sample prediction on the model; that is, train the model on some data and test it on different data.  
...
## In-Sample Prediction {#in-sample}
...
Before starting this section, complete the foreword for the Model Validation chapter (done).  
...
The goal of writing this document is to instruct the readers on how to perform data analysis using fundamental machine learning concepts.  

Focus on demonstrating the methodology  
...
Then we train the logistic regression model using the training part of the data:
...
Next, we use the trained model to predict the testing part of the data. The function `predict.glm` allows us to fit the generalized linear model (GLM) on new data. The output is a numeric vector of probabilities corresponding to each record in the testing part.   
...
We can also produce the percentage version.
...
# Also consider making part of it into a function ??
```
...
partition_list[[1]] = ind_permute[1:k_fold[1]]
for (ii in 2:length(k_fold)) {
  start = k_accumulate[ii-1]+1
  end = start + k_fold[ii] - 1
  partition_list[[ii]] = ind_permute[start:end]
}
...
train_kfold_model = glm(CS_65up ~ HighSchool_PR, data=train_kfold_data, family="binomial")
# summary(train_kfold_model)

test_kfold_prob = predict.glm(train_kfold_model, test_kfold_data, type="response")
# type="response" gives the predicted probabilities

# test_kfold_prob

# Next step: Start a for loop: exclude from 1 to 10
# But how to combine the test_kfold_prob probabilities?
# e.g. Create another numeric vector for the combined test results.
#      Then paste the test_kfold_prob values to the corresponding indices.

```

Test change for git commits.
