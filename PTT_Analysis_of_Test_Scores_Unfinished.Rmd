...
- Confusion matrix + extreme data or results
...
After implementing the logistic regression model, we also validated the results using in-sample prediction (Chapter \ref{validation}) and out-of-sample prediction (Chapter \ref{out-of-sample}). The accuracy is approximately 70% and consistent across both methods of prediction. About half of the respondents obtained a college entrance exam score at least 65, so the baseline is around 50% like a coin-flip. Compared with the baseline, the logistic regression model is doing much better. The detailed metrics are listed and explained in Section \ref{cmp-results}.  

\section*{\textcolor{red}{Unfinished below}}  

We also examined the data breakdown by high school entrance exam score category, because a model may have different levels of performance in different ranges of the independent variable. Exploratory data analysis shows the data are left-skewed, i.e., much more respondents with high scores than low scores. Given the findings in the linear regression, we performed a deep dive of the top scorers in Chapter \ref{explore-top}.    

Then after we implemented the logistic regression model, we also examined the model results by high school entrance exam score category in Section \ref{another-breakdown}. Since this is a binary classification model, we focused on the confusion matrices of each category.  

What are the model results?  

PR = percentile rank  

- PR 0-79: All predictions are FALSE.

- PR 80-89: All predictions are FALSE.

- PR 90-94: Predictions consist of both TRUE and FALSE.

- PR 95-99: All predictions are TRUE.  
