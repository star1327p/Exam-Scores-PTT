...
data = read.csv("../Data_from_PTT/ptt_SENIORHIGH_data.csv")
names(data)[1] = "pttID"
...
When all predicted values are `FALSE`, the confusion matrix is missing a `TRUE` column and we need to add it back. Similarly, When all predicted values are `TRUE`, the confusion matrix is missing a `FALSE` column and we also need to add it back. Finally, we also revert the order of `FALSE` and `TRUE`, making the output consistent with the other 2x2 confusion matrices.  

By checking the dimensions of the confusion matrices and modifying if necessary, we obtain all four 2x2 confusion matrices for the four categories of **HighSchool_PR**.
