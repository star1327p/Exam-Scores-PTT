...
Here we visualize some bivariate patterns for different values of the correlation coefficient $\rho$. Note that we hide the code that generated these graphs to avoid obscuring the topic, and the interested readers can find the code in the raw `.Rmd` files. We will discuss more about linear regression in Chapter \ref{linear-reg}.     
...
- For $\rho = 1$, the two variables $X, Y$ have a perfect linear relationship with a positive slope. In mathematical terms, we can write $Y = \alpha + \beta X$ with $\beta > 0$.  

- For $\rho = -1$, the two variables $X, Y$ have a perfect linear relationship with a negative slope. In mathematical terms, we can write $Y = \alpha + \beta X$ with $\beta < 0$.   
...
\textcolor{red}{Need to describe the graphs for a good but imperfect linear relationship.}   
...
\textcolor{red}{Need to change the graph size, so that we can put multiple rows of graphs in the same code chunk and in the same page.}     

Mid-correlation graphs   
