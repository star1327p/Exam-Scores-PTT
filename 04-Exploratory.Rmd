...
\textcolor{red}{Work on the graphical examples for the correlation coefficient}   

\textcolor{red}{Start by creating a 2x4 set of empty graphs, just like pp.311 (Section 8.1) in OpenIntro Statistics.}      
...
The histogram for **College_Score** also shows a left-skewed distribution, but not as extreme as **HighSchool_PR**. Note that right-skewed distributions are more common in real life, such as employee salaries and movie ticket sales.^[<https://www.statology.org/positively-skewed-distribution-examples/>]       
...
The strength of linear relationship can be interpreted as below:^[<https://bit.ly/3AUI8PA>]  
...
Here we visualize some bivariate patterns for different values of the correlation coefficient $\rho$, with hypothetical data and examples. Note that we hide the code that generated these graphs to avoid obscuring the topic, and the interested readers can find the code in the raw `.Rmd` files. We will discuss more about linear regression in Chapter \ref{linear-reg}.     
...
The first row of graphs show what $|\rho| = 1$ looks like.

- For $\rho = 1$, the two variables $X, Y$ have a perfect linear relationship with a positive slope. In mathematical terms, we can write $Y = \alpha + \beta X$ with $\beta > 0$ because the sign of $\beta$ controls the direction of the pattern. One example is people's height in cm and height in inches, where the two variables are a linear combination of each other.     

- For $\rho = -1$, the two variables $X, Y$ have a perfect linear relationship with a negative slope. In mathematical terms, we can write $Y = \alpha + \beta X$ with $\beta < 0$ because the sign of $\beta$ controls the direction of the pattern. One example is you eat from a box of 12 cookies. The cookies you ate and the cookies remaining should add up to 12, so they have a perfect negative linear relationship.   

```{r perfect-linear-correlation, fig.height=4, fig.width=8, echo=FALSE}
par(mfrow=c(1,2))
...
In the second row, the two variables have a good but imperfect linear relationship. We add a small random noise with mean zero to the data generative process, and the equation becomes $Y = \alpha + \beta X + \epsilon$, where $\epsilon \sim N(0,\sigma^2)$. We assume the random noise $\epsilon$ follows a normal distribution, although this is not always necessary.       
...
```{r high-correlation, fig.height=4, fig.width=8, echo=FALSE}
par(mfrow=c(1,2))
...
\textcolor{red}{Mid-correlation graphs}      

$\rho = 0.64$   

One example is students' midterm exam scores and final exam scores. Students who did well on the midterm often continue to do well on the final, but this is not guaranteed. Exam scores often have more variability than people's height and weight.        

$\rho = -0.47$   

One example is adults' muscle mass and their age. Older adults tend to have less muscle than younger ones, but it is well-known that strength training helps preserve some muscle mass.   

```{r mid-correlation, fig.height=4, fig.width=8, echo=FALSE}
par(mfrow=c(1,2))
...
\textcolor{red}{Next steps: Generate the low correlation graphs.}   

\textcolor{red}{This may come from both x, y are random.}   

For $\rho = 0$, create one graph with independence and the other graph with an obvious non-linear relationship.   

e.g. Parabola curve: $y = -x^2 + 25$, set xlim=c(-5,5)    

```{r low-correlation, fig.height=4, fig.width=8, echo=FALSE}
par(mfrow=c(1,2))
