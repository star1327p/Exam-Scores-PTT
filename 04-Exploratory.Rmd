...
\textcolor{red}{State the definition of correlation coefficient in the beginning, and \textbf{why} we would like to use this measure.}    
...
- $0.4 < |\rho_{X,Y}| < 0.6$: Moderate linear relationship
...
Also explain why we decided to hide the code that generated these graphs.  

Avoid obscuring the topic of correlation coefficient.   
...
linear_corr_gen <- function(noise_sd, relationship="positive", seed=1000){
  ...
  # print(cor(x,z))
  plot(x, z, xlab="", ylab="", main=bquote(~ rho == .(rho_rnd)))
...
```{r perfect-linear-correlation, echo=FALSE}
par(mfrow=c(1,2))
...
# \rho_{X,Y} = -0.5
linear_corr_gen(noise_sd=12, "negative", 23)
```
...
```{r low-and-zero-correlation, echo=FALSE}
par(mfrow=c(1,2))
