...
- The left image shows a bivariate scatter plot with very little correlation. For a small $\rho$ in absolute value, the two variables barely have any linear relationship. Although the bivariate scatter plot looks random, the correlation coefficient may not be exactly zero.    

Need a real-life example.   

- The right image has $\rho = 0$, i.e., no linear relationship at all. However, the graph shows a parabola curve, so the two variables have some non-linear relationship.   
