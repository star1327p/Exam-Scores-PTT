...
\textcolor{red}{Write the foreword here}   

Here we are going to visualize bivariate patterns that show different values of the correlation coefficient $\rho$. Note that we conceal the code that generated these graphs to avoid obscuring the topic, and the interested readers can find the code in the raw `.Rmd` files.      
...
linear_corr_gen <- function(noise_sd, relationship="positive", seed=1000){
  ...
  # print(cor(x,z))
  plot(x, z, main=bquote(~ rho == .(rho_rnd)), xlab="", ylab="")
...
Add graphical examples to show each $\rho$.  

$\rho = 1$ example: $y = ax+b$, with $a > 0$.  

$\rho = -1$ example: $y = ax+b$, with $a < 0$.  

\textcolor{red}{Need to decide what to do with a good but imperfect linear relationship.}   

Add $\epsilon \sim N(0,\sigma^2)$ as the random noise with mean zero.  

```{r high-linear-correlation, fig.height=8, fig.width=7, echo=FALSE}
par(mfrow=c(2,2))
...
```{r mid-and-low-correlation, fig.height=8, fig.width=7, echo=FALSE}
par(mfrow=c(2,2))
...
plot(x,y,main=bquote(~ rho == .(rho_current)), xlab="", ylab="")
