...
```{r perfect-linear-correlation, fig.height=4, fig.width=8, fig.cap="Perfect linear correlation: $\\rho = 1$ (left) and $\\rho = -1$ (right)", echo=FALSE}
...
The third set contains graphs that visualize a medium level of linear correlation, where $0.4 < |\rho| < 0.6$. The linear trend is still visible, but with more fluctuation than in the high-correlation graphs.    
...
The final set of graphs show what little-to-no linear correlation looks like. We need to remember that zero correlation does not mean the two variables are independent. They may have a non-linear relationship, which cannot be detected by the correlation coefficient.       
