...
AUC is between 0 and 1 like a probability. A model that is 100% correct has an AUC of 1, and a model that is 0% correct has an AUC of 0.   

Baseline should be a random guess (50% correct like a coin flip), and AUC is 0.5 ??   

In this way, the model performance is quantified by the AUC.     
...
roc.plot(obs_outcomes,pred,xlab = "FPR", ylab="TPR",show.thres = FALSE, main=paste("ROC Curve: AUC =", round(auc_value,2)))
```
