...
When the probability threshold is 0.7, the precision increased from 67% to 82%, but the recall decreased from 78% to 52%. A higher probability threshold means the model is less likely to predict a datapoint to be True, so a positive prediction is more likely to be actually positive, increasing the precision. On the other hand, the model may miss more datapoints (i.e., predict as negative) which are actually positive, resulting in a drop in the recall.   
...
When the probability threshold is 0.3, the recall increased from 78% to 89%, but the precision decreased from 67% to 59%.   

A lower probability threshold means the model is more likely to predict a datapoint to be True, then ???   
