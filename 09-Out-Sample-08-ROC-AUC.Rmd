...
\subsection*{\textcolor{red}{Below: Go into the Appendix ?!}}  

\textcolor{red}{Beware of the predictive probabilities! They need to be between 0 and 1, i.e., no negative values are allowed.}   

As a hypothetical example, the first 500 observations are 0 and the remaining 500 observations are 1. We also assume a model: For an observed 0, the model predicts it as average 25% chance to be 1. For an observed 1, the model predicts it as average 75% chance to be 1.   

Normal distribution to generate some random noise.  

Then use another uniform distribution to truncate ?!   

Need to explain `pmax` and `rnorm` and `runif` functions.  

\subsection*{\textcolor{red}{Above: Go into the Appendix ?!}}  
...
- p-value: Against the null hypothesis $H_0$ as random guess (AUC = 0.5).  
...
pred_prob_1 = pmax(rnorm(group_size, mean=0.25,sd=0.3), runif(group_size,min=0,max=0.01))
pred_prob_2 = pmax(rnorm(group_size, mean=0.75,sd=0.3), runif(group_size,min=0,max=0.01))
...
The `R` package `pROC` (citation needed) contains many methods to smooth the ROC curve. But `pROC` generates the ROC curve based on specificity and sensitivity, rather than FPR and TPR.   
