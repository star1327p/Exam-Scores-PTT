...
There are 6 missing values in **HighSchool_PR** and 3 missing values in **College_Score**, so each of them is recorded as "-1" (an invalid numerical value for the scores).   
...
Let's clean up the data. For **HS_Inacc** and **CS_Inacc**, we set the missing values to "0" because the two flags are binary indicators. We add two more indicators to reflect invalid (impossible) values in **HighSchool_PR** and **College_Score**.  
...
We need to remove invalid scores, not just the missing ones.   

e.g. An invalid score like **HighSchool_PR** 100 should also be treated as missing.  
...
\textcolor{red}{Consider moving the Remark to the foreword of the chapter}       
...
Pointer to record linkage methods \cite{dusetzina2014overview} ?!   
