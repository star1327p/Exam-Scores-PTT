...
We can create a demonstrative ROC plot using the `roc.plot` function in the `R` package `verification`, but we need some simulated data first. As a hypothetical example, the first 500 observations are 0 and the remaining 500 observations are 1. The vector array `obs_outcomes` is the actual observed binary outcomes. This array consists of elements only 0's and 1's.      
...
Need to explain the vector arrays `pred_prob_0` and `pred_prob_1`.   

The vector array `pred` is the predicted probabilities for each datapoint. This array consists of probability values, i.e., each value is between 0 and 1.   

Need to explain the functions: `pmax`, `pmin`, `rnorm`, and `runif`.    

The `R` function `runif` uses the uniform distribution, so it is pronounced as "r-unif" instead of "run-if".  
...
- p-value: Against the null hypothesis $H_0$ as random guess (AUC $\approx$ 0.5).  

Predictive probabilities

```{r auc-demo-draft-gen, warning=FALSE, message=FALSE}
set.seed(3333)
...
Check that all output probability values are valid.  

```{r auc-demo-draft-check}
...
Plot the demonstrative ROC curve   

```{r auc-demo-draft-roc}
