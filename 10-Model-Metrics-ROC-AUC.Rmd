...
Now let's make the demonstrative ROC curve using `roc.plot`, which plots the false positive rate (FPR) against the false negative rate (FNR). The first input vector is the actual observed outcomes `obs_outcomes`, and the second input vector is the predicted probabilities `pred`. The graph should be a concave down shape from the bottom left (0,0) to the top right (1,1). We store the output of `roc.plot` in a new object `new_test` for future use, which is of `roc.data` type.   
...
We can access the model information using the `roc.vol` label.  
...
The `roc.vol` label:  
- AUC (area under the curve) is under the column `Area`.    
- p-value: Against the null hypothesis $H_0$ as random guess (AUC $\approx$ 0.5).   
- `binorm.area`: This applies only if we select a binormal fit for the ROC curve, which is not the case here.   
